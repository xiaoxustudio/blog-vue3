        ...
        <div class="flex-1">
            ...
            <el-skeleton :loading="is_ready" animated>
                <template #default>
                    <div class="title text-2xl font-bold">
                        <span v-if="!is_editor.title_enable" @dblclick="handleTitle">{{
                            _Article?.article_title
                        }}</span>
                        <span v-if="is_editor.enable && is_editor.title_enable" class="w-full"
                            ><el-input
                                ref="titleRef"
                                type="text"
                                v-model="_Article_duplicate.title"
                                class="w-full"
            ...
            <el-scrollbar class="w-full" height="95vh">
                <div
                    ...
                    v-if="!is_editor.content_enable"
                    @dblclick="handleContent"
                    v-html="_Article?.article_content"
                ></div>
                <div
                    class="h-full w-full text-gray-800"
                    v-if="is_editor.enable && is_editor.content_enable"
                >
                    <el-input
                        ref="contentRef"
                        type="text"
                        v-model="_Article_duplicate.content"
                        class="h-full w-full"
            ...
            <div class="bottom py-4">
                <div class="tags mb-2 flex gap-5">
                    <el-tag :key="index" v-for="(item, index) in Tags">{{ item }}</el-tag>
                </div>
            ...
            <div class="relative block" v-if="userInfo.user_id == _Article?.author_id">
                <div class="float-right">
                    <el-button @click="handleEnableEdit">修改文章</el-button>
                </div>
            </div>
            <div class="comment">
                <el-skeleton :loading="is_ready" animated>
...
const Tags = computed(
    () => _Article.value?.article_tags?.split('|').filter((_val) => _val.length > 0) ?? [],
);
...
const _Article_duplicate = reactive({
    title: '',
    content: '',
});

const is_ready = ref(true);
const is_editor = reactive({
    enable: false, // 总开关
    title_enable: false, // title开关
    content_enable: false, // content开关
});

const handleEnableEdit = () => {
    is_editor.enable = !is_editor.enable;
    if (is_editor.enable) {
        ElMessage.success(`${is_editor.enable ? '修改' : '关闭修改'}模式`);
    } else {
        ElMessage.info(`${is_editor.enable ? '修改' : '关闭修改'}模式`);
    }
};
// 标题修改
const handleTitle = () => {
    if (is_editor.enable) {
        is_editor.title_enable = true;
        nextTick(() => {
            if (titleRef.value) {
...
const handleTitleFocusOut = () => {
    is_editor.title_enable = false;
    if (titleRef.value && _Article.value?.article_title !== _Article_duplicate.title) {
        ElMessageBox.confirm('文章标题已更改，是否保存修改?', '提示', {
            ...
            .catch(() => {
                _Article_duplicate.title = _Article.value?.article_title || '';
            });
    }
};
const submitChangeTitle = async () => {
    await postData(`?type=${API_Type.u_article}`, {
        token: userInfo.token,
        username: userInfo.username,
        tname: 'article_title',
        tvalue: _Article_duplicate.title,
        id: _Article.value?.article_id,
    }).then((data) => {
...
const handleContentFocusOut = () => {
    is_editor.content_enable = false;
    if (contentRef.value && _Article.value?.article_content !== _Article_duplicate.content) {
        ElMessageBox.confirm('文章内容已更改，是否保存修改?', '提示', {
            confirmButtonText: '修改',
            cancelButtonText: '取消',
        })
            .then(() => {
            ...
            .catch(() => {
                _Article_duplicate.content = _Article.value?.article_content || '';
            });
    }
};
const submitChangeContent = async () => {
    await postData(`?type=${API_Type.u_article}`, {
        token: userInfo.token,
        username: userInfo.username,
        tname: 'article_content',
        tvalue: _Article_duplicate.content,
        id: _Article.value?.article_id,
    }).then((data) => {
        const _data = data.data;
        if (_data.status) {
